...
$autoDisc = getDiscrepancyByAccession($_GET['acc'], "AutoDiscrepancy");
$adminDisc = getDiscrepancyByAccession($_GET['acc'], "AdminDiscrepancy");
$compDisc = getDiscrepancyByAccession($_GET['acc'], "CompositeDiscrepancy");
...
$edNotified = getDiscrepancyByAccession($_GET['acc'], "EDNotify");
?>
...
<?php tableStartSection("Tag and Review", 0, (in_array($compDisc, $discrepancyReviewRequired) || $edNotified )?False:False);?><br><!--Set to False:True if you want the tags menu to collapse -->
<div class='control' style='background:none' id="review">
<?php 

include "tag_navigator.php";
if (interpretedByTrainee($_GET['acc'], $_SESSION['traineeid']))  {
    $acc = $_GET['acc'];
    echo <<< END
...
END;
    ...
    if (in_array($compDisc, $discrepancyReviewRequired) || $edNotified)  {
        $selected = ['', '' ,'' ,''];
