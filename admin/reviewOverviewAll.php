...
$sql = "
SELECT
IF(em.ResidentYear>4, 'Fellow', 'Resident') AS `Group`,
    COUNT(*) as 'Total Volume',
#    SUM(IF(em.Location='E', 1, 0)) AS `ER Vol`,
#    SUM(IF(em.Location='I' AND (em.Urgency LIKE 'S%'), 1, 0)) AS `Inpt STAT Vol`,          
    SUM(IF(ed.CompositeDiscrepancy='MajorChange',1,0)) AS 'Major Change',
    FORMAT(SUM(IF(ed.CompositeDiscrepancy='MajorChange',1,0))*10000 / COUNT(*)/100,2) AS 'Major %'
