...
while ($count > 0) {
    ...
    //}
    //echo "Done with RIS, now updating Capricorn...\n";
    //flush_buffers();
    //foreach ($studies as $s) {
        /*
        // Looking up CPT code in GE RIS is expensive.  Only do it if necessary.
        $sql = "SELECT CPTCode from ExamCodeDefinition WHERE ExamCode=\"" . $s['ExamCode'] . "\" AND ORG=\"" . $s['Organization'] . "\";";
        $results = $resdbConn->query($sql) or die (mysqli_error($resdbConn));
        $results = $results->fetch_array();
        if ($results[0] == NULL || $results[0] == "")  {
            $sql = "SELECT DISTINCT PrCode FROM vusrBillingReport as billing INNER JOIN vDxRptContributingResponsible as report ON (billing.InternalExamID=report.ActivityHeaderID) INNER JOIN vRptChargeCPT4PRoc as cpt ON (cpt.ChargeCodeDE=billing.ChargeCodeID) WHERE AccessionNumber='" . $s['AccessionNumber'] . "'";
            $cptResult = sqlsrv_query($conn2, $sql);
            if ($cptResult) {
                $codes = array();
                while ($cpt = sqlsrv_fetch_array($cptResult, SQLSRV_FETCH_ASSOC)) {
                    $codes []= trim($cpt['PrCode']);
                }
                $codes = implode("|", $codes);
            }
            if (trim ($codes) != "") {      // If no CPT code is assigned, then no point in updating.
                echo "New CPT Code. " . $codes . " matched to " . $s['ExamCode'] . " at site " . $s['Organization'] . "\n";
                $sql = "UPDATE ExamCodeDefinition SET CPTCode='" . $codes . "' WHERE ExamCode='" . $s['ExamCode'] . "' AND ORG='" . $s['Organization'] . "';";           
